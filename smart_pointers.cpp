...
int main() {
	...
	// Shared pointer are valid while at LEAST one shared pointer
	...
	cout<<"Use count of shared_int: "<<shared_int_2.use_count()<<endl;
	...
	// Convert the weak pointer to a shred pointer and print again
	auto shared_int_3 = weak_int.lock();
	cout<<"Use count of shared_int: "<<shared_int_3.use_count()<<endl;

	// Destroy the memory for 2 of the shared ponters and check uniqueness
	shared_int_1.reset();
	shared_int_2.reset();
	cout<<"Use count of shared_int: "<<shared_int_3.use_count()<<endl;
